...
//        this.style.cssText = this.#style()
    ...
    #pushStart() {
        ...
        //this._timeout.hold = setTimeout(()=>{this.dispatchEvent(new CustomEvent('hold'));this._push.start=0;this._push.end=0;this.#setStyleFree();}, this._push.hold)
        //this._timeout.hold = setTimeout(()=>{this._push.start=0;this._push.end=0;this.#setStyleFree();this.#setStyleRiveted();this.dispatchEvent(new CustomEvent('hold'));}, this._push.hold)
...
//        if (0===this._device.keyboard.push.end) { this._device.keyboard.push.end = Date.now() }
    ...
    /*
    #updateState() {
        if (this.States.isHide===this._state) {this.#updateStyle();return;}
        else if (this.States.isDisabled===this._state) {this.#updateStyle();return; }
        else if (this.#isRiveted()) { this._state = ((this.#isDown()) ? this.States.isDown : this.States.isRiveted);this.#updateStyle();return; }
        else if (this.#isDown()) { this._state = this.States.isDown;this.#updateStyle();return; }
        else { this._state = this.States.isFree;this.#updateStyle();return; }
