...
//        attr.onmousedown = (e)=>{console.log('mousedown');this.#pushStart();this._device.mouse.isDown=true;this.#updateState();}
//        attr.onmouseup = (e)=>{console.log('mouseup');this._device.mouse.isDown=false;this.#updateState();this.#onPushed(e);}
    ...
    //#clearTimeoutLongPushed() { if (this._longPushed) { clearTimeout(this._longPushed) } }
...
//    #onPushed(e) { this._pushed.end=Date.now(); if (!this.#isLognPushed()) { this.options.onPushed(e); this.#clearTimeoutLongPushed(); } this._pushed.start=Date.now(); console.log(e); }
    #onPushed(e) {
        ...
        if (this.options.isOnce) {
            ...
            if (this.#isDuplicatePushed()) { // 超短時間に２回以上実行要求されたら
                ...
                this._timeout.disabled = setTimeout(()=>{ this._isDisabled=false; this.#clearOnceTime(); })
                this._isDisabled = true
//                if (this._setTimeoutDisabled) { clearTimeout(this._setTimeoutDisabled); }
//                this._setTimeoutDisabled = setTimeout(()=>{ this._isDisabled=false; this.#clearOnceTime(); })
                 //this._setTimeoutDisabled = setTimeout(()=>{this._state=this.States.isDisabled; })
                //this._state = this.States.isDisabled
                //this.#clearOnceTime()
            } else { // まだ一回しか要求されてない、２回以上要求されたが一定時間以上の間隔がある
                this.options.onPushed(e)
                if (this._timeout.disabled) { clearTimeout(this._timeout.disabled); }
                this._timeout.disabled = setTimeout(()=>{ this._isDisabled=false; this.#clearOnceTime(); })
                this._isDisabled = true
...
//        if (0 === this._pushed.once.last.start) { return false }
//        if (0 === this._pushed.once.last.end) { return false }
    ...
    #setOnceStart() {
        ...
        else if (0===this._pushed.once.last.start){ this._pushed.once.last.start = Date.now() }
    }
    #setOnceEnd() {
        if (0===this._pushed.once.first.end) { this._push.once.first.end = Date.now() }
        else if (0===this._pushed.once.last.end) { this._push.once.last.end = Date.now() }
    }
    */
    #clearOnceTime() {
